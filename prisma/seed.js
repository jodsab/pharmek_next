...
async function main() {
  ...
  try {
    ...
    if (!prod) {
      prod = await prisma.products.create({
        data: {
          ...
          dosis_y_via:
            "3 - 6 mL/ kg de p.v. dependiendo del pelaje del animal. Aplicación tópica.",
    ...
    if (!prod) {
      prod = await prisma.products.create({
        data: {
          nombre: "Aifen 10%",
          composicion:
            "Cada mL contiene: Ketoprofeno 100 mg Excipiente c.s.p. 1 mL",
          indicaciones:
          ...
          animal_mayor_menor: "Mayor y Menor",
          presentaciones:
            "Frascos de vidrio x 10 mL, 20 mL, 50 mL, 100 mL y 250 mL",
        },
      });
    ...
    for (const productKey in productDirectoryMapping) {
      ...
      console.log(
        `  Procesando directorio para "${product.nombre}" (${subdirectoryName})...`
      );
      ...
      try {
        ...
        console.log(
          `    Encontrados ${entries.length} elementos en "${subdirectoryName}"`
        );
        ...
        const imageFilesPromises = entries.map(async (entry) => {
          ...
          try {
            ...
            if (
              stat.isFile() &&
              /\.(webp|jpg|jpeg|png|gif|svg)$/i.test(entry)
            ) {
        ...
        files = (await Promise.all(imageFilesPromises)).filter(
          (file) => file !== null
        );

        console.log(
          `    Filtrados a ${files.length} archivos de imagen en "${subdirectoryName}".`
        );
      } catch (error) {
        if (error.code === "ENOENT") {
          ...
          console.error(
            `    Error leyendo el directorio "${subdirectoryName}":`,
            error
          );
        }
    ...
    if (products.trakLa) {
      ...
      if (!destacado) {
        ...
        const trakLaMainImage =
          allProductImages[publicPathPrefix + "TrakLa/trakla1.webp"]; // <-- Usa la URL exacta esperada
        ...
        console.log(
          `  Producto destacado para ${products.trakLa.nombre} (Orden 1) creado.`
        );
      } else {
        console.log(
      ...
      console.log(
        "  Producto Trak LA 1% no encontrado, no se puede crear destacado Orden 1."
      );
    }
    ...
    if (products.dermicare) {
      ...
      if (!destacado) {
        const dermicareMainImage =
          allProductImages[publicPathPrefix + "Dermicare/dermicare1.webp"]; // <-- URL esperada
        ...
        console.log(
          `  Producto destacado para ${products.dermicare.nombre} (Orden 2) creado.`
        );
      } else {
        console.log(
      ...
      console.log(
        "  Producto Dermicare6 no encontrado, no se puede crear destacado Orden 2."
      );
    }
    ...
    if (products.pracanex) {
      ...
      if (!destacado) {
        const pracanexMainImage =
          allProductImages[publicPathPrefix + "Pracanex/pracanex1.webp"]; // <-- URL esperada
        ...
        console.log(
          `  Producto destacado para ${products.pracanex.nombre} (Orden 3) creado.`
        );
      } else {
        console.log(
