export async function POST(req) {
  try {
    ...
    if (!query) {
      return new Response(
        JSON.stringify({ error: "Falta el mensaje del usuario" }),
        { status: 400 }
      );
    }
    ...
    const mistralResponse = await fetch(
      "https://api.mistral.ai/v1/chat/completions",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",jkjn
    ...
    if (!mistralResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Error de Mistral: ${errorText}` }),
    ...
    const responseText =
      data.choices?.[0]?.message?.content || "Respuesta no disponible 🐱💭";
    ...
    return new Response(
      JSON.stringify({ error: "Error al obtener la respuesta. 😿" }),
      { status: 500 }
    );
  }
}
