...
export default function Chatbot(): JSX.Element {
  ...
  return (
    <div className="chatbot-container">
      <AnimatePresence>
        {isOpen && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="chatbot-overlay" />}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            aria-label="Chatbot"
          >
            <button onClick={() => setIsOpen(false)} className="chatbot-close-button" aria-label="Cerrar chat">
              <IoMdClose size={20} />
            </button>
            ...
            <div className="chatbot-messages-area" role="log" aria-live="polite">
              {messages.map((msg, i) => (
                <div key={i} className={`message-row ${msg.sender === 'bot' ? 'bot' : 'user'}`}>
                  {msg.sender === 'bot' && <Image src={gatito} alt="Bot" width={32} height={32} className="message-avatar" />}
                  <div className="message-bubble">{msg.text}</div>
                  {msg.sender === 'user' && <Image src={perfil} alt="Usuario" width={32} height={32} className="message-avatar" />}
                </div>
              ))}
            ...
            <div className="chatbot-input-area">
              <input
                ...
                aria-label="Mensaje"
              />
              <button onClick={handleSendMessage} className="chatbot-send-button" disabled={isSending} aria-label="Enviar mensaje">
                <IoMdSend size={20} />
              </button>
            </div>
          </motion.div>
      ...
      <button onClick={() => setIsOpen((v) => !v)} className="chatbot-toggle-button" aria-label={isOpen ? 'Cerrar chat' : 'Abrir chat'}>
